...
use eframe::egui::{vec2, Color32, ColorImage, FontId, RichText, Vec2};
...
pub fn plot_matrix(
    ...
    ui.horizontal(|ui| {
        ...
        colorbar(
            ui,
            &(0.1 * width as f64 * size as f64),
            &(0.78 * height as f64 * size as f64),
        );
        ui.vertical(|ui| {
            for i in 0..11 {
                ui.label(
                    RichText::new(format!("{:.1}%", (i as f64) / (11.0 - 1.0) * (100.0)))
        ...
        if plot_response.response.clicked() {
                ...
                && pixel_selected.selected
            {
                ...
                pixel_selected.y = height as f64 - 1.0 - val.y.floor();
                pixel_selected.id =
                    id_matrix[pixel_selected.x as usize][pixel_selected.y as usize].clone();
            }
        }
    });
    ...
    ui.horizontal(|ui| {
        ...
        let plot_response = plot.show(ui, |plot_ui| {
            ...
            let selected_row = PlotPoints::from(vec![
                ...
                [
                    width as f64 - 1.0,
                    height as f64 - pixel_selected.y.floor() - 1.0,
                ],
