use eframe::egui::{Sense, Ui, Widget};
use eframe::emath;
...
impl<'a> DoubleSlider<'a> {
    pub fn new(
        ...
        DoubleSlider {
            ...
            color: Color32::GRAY,
            stroke: Stroke::new(4.0, Color32::RED.linear_multiply(0.5)),
            range,
        }
    }
...
impl<'a> Widget for DoubleSlider<'a> {
    fn ui(self: Self, ui: &mut Ui) -> eframe::egui::Response {
        ...
        if !self.inverted_highlighting {
            ...
            let points_in_screen: Vec<Pos2> =
                [self.control_point_radius, self.to_coord(*self.left_slider)]
                    .iter()
                    .map(|p| {
                        to_screen
                            * Pos2 {
            ...
            let points_in_screen: Vec<Pos2> = [
                self.to_coord(*self.right_slider),
                self.width - self.control_point_radius,
            ]
            .iter()
            .map(|p| {
                to_screen
